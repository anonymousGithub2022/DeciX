...
from .myhelper import get_position, SOS_ID, EOS_ID, PAD_ID
...
class RNNEncoder(nn.Module):
    def __init__(self, embedder, char_embedder, input_size, hidden_size, bidir, n_layers, dropout=0.5,
                 noise_radius=0.2):
        ...
        self.init_h = nn.Parameter(torch.randn(self.n_layers * (1 + self.bidir), 1, self.hidden_size),
                                   requires_grad=True)  # learnable h0
        self.init_weights()
    ...
    def forward(self, inputs, char_inputs=None, input_lens=None, init_h=None, noise=False):
        inputs = self.get_embedding(inputs, char_inputs)
        return self.forward_embedding(inputs, input_lens, init_h, noise)

    def get_embedding(self, inputs, char_inputs=None):
    ...
    def forward_embedding(self, inputs, input_lens=None, init_h=None, noise=False):
        ...
        if init_h is None:
            init_h = self.init_h.expand(-1, batch_size,
                                        -1).contiguous()  # use learnable initial states, expanding along batches
        ...
        h_n = h_n.view(self.n_layers, (1 + self.bidir), batch_size,
...
class Attention(nn.Module):
    ...
    def forward(self, Q, K, V, key_pad_mask=None, attn_mask=None):
        ...
        if self.attn_combine:
            ...
            combined_attn_ctx = torch.tanh(self.linear_comb(combined.view(-1, self.q_dim + self.v_dim))).view(
                batch_size, -1, self.q_dim)
...
class RNNDecoder(nn.Module):
    ...
    def forward(self, init_h, enc_hids, src_pad_mask, context=None, inputs=None, lens=None):
        ...
        if self.use_attention:
            attn_ctx, _, _ = self.attn(init_h.unsqueeze(1), enc_hids, enc_hids,
                                       src_pad_mask)  # initial attention context
            for di in range(maxlen):
                ...
                decoded = logits if di == 0 else torch.cat([decoded, logits],
                                                           1)  # decoded: [batch_sz x maxlen x vocab_sz]
        else:
    ...
    def sample_likehood(self, init_h, enc_hids, src_pad_mask, maxlen):
        """
        A simple greedy sampling
        :param init_h: [batch_sz x hid_sz]
        :param enc_hids: a tuple of (enc_hids, mask) for attention use. [batch_sz x seq_len x hid_sz]
        ...
        decoded_words = torch.zeros((batch_size, maxlen), dtype=torch.long, device=device)
        decoded_likehood = torch.zeros((batch_size, maxlen, self.vocab_size), dtype=torch.float32, device=device)
        ...
        for di in range(maxlen):
            ...
            #     x = torch.multinomial(F.softmax(logits, dim=1), 1)  # [batch_size x 1 x 1]?
            decoded_words[:, di] = x.squeeze()
            decoded_likehood[:, di] = logits
            len_inc = len_inc * (x.squeeze() != EOS_ID).long()  # stop increse length (set 0 bit) when EOS is met
            sample_lens = sample_lens + len_inc

        decoded_words = decoded_words.data.cpu().numpy()
        sample_lens = sample_lens.data.cpu().numpy()
        # decoded_likehood = [d[:l] for d, l in zip(decoded_likehood, sample_lens)]
        return decoded_likehood
        # return decoded_words, sample_lens, decoded_likehood
...
class BeamNode(object):
    ...
    def __lt__(self, other):
        ...
        print("[warning] two nodes have the same score (%d-%d), possible recursive (cycle) search." % (
        self.wordid, other.wordid))
